<?php

namespace App\Services;


use Google_Client;
use Google_Service_Sheets;
use Google_Service_Sheets_ValueRange;
use Google_Service_Sheets_Spreadsheet;
use Google_Service_Drive_Permission;
use Google_Service_Drive;
use Google_Service_Drive_DriveFile;
use App\product;
use Google_Service_Drive_ParentReference;
class GoogleSheet
{
    private $spreadSheetId;

    private $client;

    private $googleSheetService;

    public function __construct()
    {

        
        $this->spreadSheetId = '1TCn8gzd46k_eCuK4PUnI_R04otgj4jJVcOiQRgkm7cI';

        $this->client = new Google_Client();
        $this->client->setAuthConfig(storage_path('credentials.json/armadio-335715-310bc29d1909.json'));
        $this->client->addScope("https://www.googleapis.com/auth/spreadsheets");
        $this->client->addScope("https://www.googleapis.com/auth/drive");
        $this->client->addScope("https://www.googleapis.com/auth/drive.file");

        
        $this->googleSheetService = new Google_Service_Sheets($this->client);
        $this->googlePermission = new Google_Service_Drive($this->client);
    }



    public function upload($data){

        $file = new Google_Service_Drive_DriveFile($this->client);
            $image_tmp = $data;
                  
                      // Upload Images after Resize
                      $extension = $image_tmp->getClientOriginalExtension();
                      $fileName = rand(111,99999).'.'.$extension;
                     
                  

        $file->setMimeType('pdf');
        $file->setName('ddd');
        $file->setDescription('dddd');
    $file->setParents('172Khcq21SJ47kuk9e-zVw3_McJCDSZIJ');
$service = new Google_Service_Drive($this->client);




        $createdFile = $service->files->create($file, array(
            'data' => file_get_contents($data),
          
          ));


          $userPermission = new Google_Service_Drive_Permission(array(
            'type' => 'anyone',
            'role' => 'reader',
            'emailAddress' => 'bh621453@gmail.com'
        ));

        $this->googlePermission->permissions->create($createdFile->id, $userPermission     
    );
    
dd($createdFile->id);

    }

    public function crtreetgooglesheet(){
        $spreadsheet = new Google_Service_Sheets_Spreadsheet([
            'properties' => [
                'title' => 'trying',
            ]
        ]);
        $spreadsheet =   $this->googleSheetService->spreadsheets->create($spreadsheet, [
            'fields' => 'spreadsheetId'
        ]);

    
        $userPermission = new Google_Service_Drive_Permission(array(
            'type' => 'user',
            'role' => 'owner',
            'emailAddress' => 'bh621453@gmail.com'
        ));

        $this->googlePermission->permissions->create($spreadsheet->spreadsheetId, $userPermission, 
        array('transferOwnership' => 'true'))
        ;
    

     $this->spreadSheetId =  $spreadsheet->spreadsheetId;


     $range = 'Sheet1!A:E';
     $values = [
         [

            'name',
            'section_id',
        'img',
        'code',
        
        'hing_id',
        'hingez',
        
        'hand_id',
               'hand',
        
        
        'dis',
        'stream_id',
        'stream',

         ]
     ];
     $body = new Google_Service_Sheets_ValueRange([
         'values' => $values
     ]);

     $params = [
        'valueInputOption' => "RAW"
    ];
    $result = $this->googleSheetService->spreadsheets_values->append($this->spreadSheetId, $range, $body, $params);
  
    }


    public function readGoogleSheet()
    {
      
        $dimensions = $this->getDimensions($this->spreadSheetId);
        $range = 'Sheet1!A1:' . $dimensions['colCount'];

      
        $data = $this->googleSheetService
            ->spreadsheets_values
            ->batchGet($this->spreadSheetId, ['ranges' => $range]);

        return $data->getValueRanges()[0]->values;
    }

    public function saveDataToSheet(array $data)
    {
        $dimensions = $this->getDimensions($this->spreadSheetId);

        $body = new Google_Service_Sheets_ValueRange([
            'values' => $data
        ]);

        $params = [
            'valueInputOption' => 'USER_ENTERED',
        ];

        $range = "A" . ($dimensions['rowCount'] + 1);

        return $this->googleSheetService
            ->spreadsheets_values
            ->update($this->spreadSheetId, $range, $body, $params);
    }

    private function getDimensions($spreadSheetId)
    {
        $rowDimensions = $this->googleSheetService->spreadsheets_values->batchGet(
            $spreadSheetId,
            ['ranges' => 'Sheet1!A:A', 'majorDimension' => 'COLUMNS']
        );

        //if data is present at nth row, it will return array till nth row
        //if all column values are empty, it returns null
        $rowMeta = $rowDimensions->getValueRanges()[0]->values;
        if (!$rowMeta) {
            return [
                'error' => true,
                'message' => 'missing row data'
            ];
        }

        $colDimensions = $this->googleSheetService->spreadsheets_values->batchGet(
            $spreadSheetId,
            ['ranges' => 'Sheet1!1:1', 'majorDimension' => 'ROWS']
        );

        //if data is present at nth col, it will return array till nth col
        //if all column values are empty, it returns null
        $colMeta = $colDimensions->getValueRanges()[0]->values;
        if (!$colMeta) {
            return [
                'error' => true,
                'message' => 'missing row data'
            ];
        }

        return [
            'error' => false,
            'rowCount' => count($rowMeta[0]),
            'colCount' => $this->colLengthToColumnAddress(count($colMeta[0]))
        ];
    }

    private function colLengthToColumnAddress($number)
    {
        if ($number <= 0) return null;

        $letter = '';
        while ($number > 0) {
            $temp = ($number - 1) % 26;
            $letter = chr($temp + 65) . $letter;
            $number = ($number - $temp - 1) / 26;
        }
        return $letter;
    }
}